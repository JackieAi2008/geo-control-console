...
function genBaikeDraft(park, F, op) {
  const g = (k, d) => F[k] || d;
  const VN = venueNoun();
  /* V0.2.2 F9：lite（楼宇）档位基本信息用楼宇口径字段（租金/入驻率/楼层等），与口径骨架一致 */
  const LITE = (typeof curProject === "function") && ((curProject() || {}).matrixTier === "lite");
  const baikeInfo = LITE
    ? `\n- 楼层数：${g("楼层数", "【待填：见口径表】")}\n- 标准层面积：${g("标准层面积", "【待填：见口径表】")}\n- 租金区间：${g("租金区间", "【待填：见口径表】")}\n- 入驻率：${g("入驻率", "【待填：见口径表】")}\n- 竣工年份：${g("竣工年份", "【待填】")}\n- 物业运营方：${g("物业运营方", "【待填】")}`
...
function opsBuildToolkit() {
  ...
  /* V0.2.2 F9：楼宇（lite）档位消费 lite 口径骨架字段（楼层数/租金区间/入驻率等）——
     此前一园一档只渲染园区档行集且租金行硬编码【待填】，楼宇项目最关键的租金数据填了也不进物料 */
  const LITE = ((curProject() || {}).matrixTier === "lite");
  const oneLiner = LITE
    ? `${park}是${op}运营的${venueNoun()}，位于${city}，租金${g("租金区间", "【待填：元/㎡/月】")}，入驻率${g("入驻率", "【待填】")}。`
    : `${park}是${op}旗下园区，位于${city}，主导${industry}产业。`;
  const infoTable = (LITE
    ? `| 项目 | 数据 | 时点 |\n|---|---|---|\n| 运营主体 | ${op} | — |\n| 详细地址 | ${g("详细地址", "【待填：口径表锚定区】")} | — |\n| 楼层数 | ${g("楼层数", "【待填】")} | — |\n| 标准层面积 | ${g("标准层面积", "【待填】")} | 【待填】 |\n| 租金区间 | ${g("租金区间", "【待填：元/㎡/月】")} | 【待填】 |\n| 入驻率 | ${g("入驻率", "【待填】")} | 【待填】 |\n| 竣工年份 | ${g("竣工年份", "【待填】")} | — |\n| 物业运营方 | ${g("物业运营方", "【待填】")} | — |`
